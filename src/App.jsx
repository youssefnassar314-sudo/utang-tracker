...
function App() {
    ...
    if (urgentItems.length > 0 && Notification.permission === "granted") {
      urgentItems.forEach(item => {
        new Notification("🚨 Utang Reminder!", {
          body: `Due na today ang ${item.category} (${item.description})!`,
          icon: "/vite.svg" 
        });
      });
    }
  ...
  const addUtang = async (e) => {
    ...
    await addDoc(collection(db, "utang"), {
      description, amount: parseFloat(amount), paidAmount: 0, category, dueDate, isPaid: false, createdAt: new Date()
    });
  ...
  const updatePartial = async (id, val) => {
    ...
    await updateDoc(utangRef, { paidAmount: pAmount, isPaid: pAmount >= item.amount });
  };
  ...
  const clearPaid = async () => {
    if(window.confirm("Buburahin na lahat ng bayad sa history?")) {
      ...
      paidOnly.forEach(async (item) => { await deleteDoc(doc(db, "utang", item.id)); });
    }
  };
